...
public class RoleAdder implements RoleGUI {
    ...
    public RoleAdder(MessageReceivedEvent evt) {
        ...
        message = evt.getChannel().sendMessageEmbeds(EmbedUtils.getGameRoleListEmbed(this, page, roleList)).complete();
    ...
    private void chooseRole(MessageReactionAddEvent evt) {
        ...
        switch (reactionName) {
            ...
            case e_left:
                if (page <= 1)
                    return;
                page--;
                message.editMessageEmbeds(EmbedUtils.getGameRoleListEmbed(this, page, roleList)).queue();
        ...
        message.editMessageEmbeds(EmbedUtils.confirmDesiredRole(this)).queue();
    }
        ...
        switch (reactionName) {
            ...
            case confirm:
                ...
                if (guild.getMemberById(user.getId()).getRoles()
                    ...
                    message.editMessageEmbeds(EmbedUtils.alreadyHasRoleEmbed(Bot.gameRoleManager.getGuildRoleManager(evt.getGuild()).getRoleFromGameRole(this.desiredRole))).queue();
                    return;
                }
                try {
                    message.editMessageEmbeds(EmbedUtils.addRoleCompleteEmbed(Bot.gameRoleManager.getGuildRoleManager(evt.getGuild()).getRoleFromGameRole(this.desiredRole))).queue();
